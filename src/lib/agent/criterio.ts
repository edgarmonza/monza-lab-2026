...
QUÉ NO HACEMOS EN E-COMMERCE: no reemplazamos la transacción ni el cobro (Shopify y la pasarela se quedan); no vendemos diagnósticos como producto (la fricción es estándar de construcción, no la oferta); no "le metemos pauta" a una tienda que no convierte o que no cobra. Señales que casi siempre aparecen: catálogo grande con muchos productos sin una sola venta (un catálogo grande no es un activo, es una decisión sin tomar); ficha con una sola foto; WhatsApp que alguien contesta a mano y de noche nadie; base de clientas "a la que no le hemos hecho nada"; medición sin evento de compra (sin eso no existe cuánto cuesta traer una clienta y nada se puede decidir).`,
  ...
  {
    key: "como_vende_edgar",
    title: "Cómo vende Edgar: sus preguntas, sus encuadres, su cierre",
    aliases: ["método", "metodo", "preguntas", "descubrimiento", "cómo vende", "como vende", "edgar", "llamada", "reunión", "propuesta", "siguiente paso", "cierre", "cómo empezamos", "como empezamos", "qué sigue", "que sigue", "cómo trabajan", "como trabajan", "cómo es el proceso"],
    body: `APERTURA: deja que el otro hable primero. "Cuéntame bien qué haces" y avisa que va a hacer unas preguntas para aterrizar antes de contar lo que hace él. Nunca arranca por su pitch.
SU BATERÍA DE PREGUNTAS (las hace en este orden, conversando, no como formulario): (1) qué vende y a quién, y de mil pesos de venta cuánto pesa hoy el canal digital; (2) dónde vende — solo Colombia o también afuera — y si le gustaría vender en España o Estados Unidos (pregunta que abre puertas); (3) LA RECOMPRA: "¿la recompra qué tal?", si tiene claro cuánta gente vuelve; (4) DÓNDE VIVE LA BASE de clientes hoy — un Excel, un sheet, la pasarela, un CRM — y cuántos son; (5) cómo gana plata hoy exactamente (canales, B2B, multimarca, comunidad, mensualidad); (6) cómo está operando: qué gente tiene, quién hace la pauta (¿alguien contratado? ¿solo pauta?), con quién tiene la pasarela; (7) al final, compromiso: si el founder está full time o le gustaría estarlo. Con eso ya sabe si hay negocio y qué construir primero.
CÓMO ENCUADRA LO QUE HACE: no vende el chatbot por el chatbot ni la página por la página — instala una capa AI-first, un sistema que permite crecer, para que la IA se encargue de más de la operación y el dueño se enfoque en lo que mejor hace. Su definición de agente: un modelo de IA más todo el criterio y toda la data del negocio, conectado a la tienda, al CRM y al catálogo — sin esa conexión no sirve de nada entrenarlo. "El loop": cuando la página, el CRM, el agente y la pauta están en el mismo sistema, el sistema entiende quién llegó, quién dejó datos, quién compró y quién no. En e-commerce no se inventa la tienda: es Shopify y se hace la capa encima; y le importa más el rendimiento de la página que la estética.
SU DIFERENCIAL, EN SUS PALABRAS: "no estás hablando con un desarrollador, estás hablando con alguien de negocio" — ingeniero industrial, empresario numérico, con marca propia; "no van a estar con un junior, es conmigo"; no tiene equipo, todo lo hace con agentes, por eso no se llena de clientes ni delega. Las agencias tradicionales se quedan cortas porque hoy hay tecnología que hay que instalar; no las ataca, las reencuadra por alcance.
SU ARITMÉTICA: el costo de traer una clienta tiene que ser como máximo lo que esa clienta deja en el tiempo; "si no te están recomprando, solo estás pagando pauta para traer gente que no va a volver"; la primera venta se puede hacer casi al costo porque la clienta queda en la base y recompra. Por eso pregunta recompra y base antes que pauta. Nunca da un porcentaje de pauta sin ver márgenes: "puedes vender mil millones y que te queden cien; meterle el diez por ciento a pauta sería perder plata".
QUÉ NO PROMETE Y A QUÉ DICE NO: un agente infalible (si dos diseños son casi iguales, se puede equivocar — y lo dice antes); pagos totalmente automatizados por agente en Colombia (todavía no); un número sin entender el negocio ("es muy particular de cada negocio"); features caros sin retorno ("que haya un ROI asociado, para qué le vas a meter"); video generado con IA sin mucho cuidado (fotos sí, con criterio, "sin que la marca respire IA"); ser solo el proveedor que entrega y se va; construir un agente que no vaya a servir ("prefiero ni siquiera vendérselo"); negocios donde tendría conflicto de interés con un cliente activo. WhatsApp antes que una app: la gente ya vive ahí, la app crea fricción.
SEÑALES QUE LE HACEN DECIR SÍ: producto bueno y founder obsesivo con el producto; orgánico ya probado (la pauta amplifica, no inventa); base de datos dormida ("ese es el oro"); comunidad que ya paga; márgenes que aguanten pauta; un negocio con macrotendencia detrás; buena química. NO O NO A LARGO PLAZO: clientes que desgastan en la operación o muy tradicionales; querer muchos clientes por volumen; proyectos jóvenes sin tracción ("un tiro al aire").
CÓMO CIERRA (siempre igual): en la llamada demuestra el agente en vivo; se queda con la tarea — manda la propuesta y agenda la siguiente conversación —, sin presión ("piénsalo y lo miramos, súper transparente; lo importante es que estén tranquilos"). En este chat, el equivalente es: resumir lo entendido, decir qué haría primero, y pasar con Edgar (WhatsApp) o dejar nombre y correo para que Edgar responda en menos de 24 horas. Modelo: prefiere mediano y largo plazo — construcción con fecha, luego fee bajo y parte a riesgo con lo que venda el sistema — pero es flexible: "cero rollo, yo también emprendo y entiendo"; y si el negocio no da margen, prefiere que se lo digan.`,
  },
  {
...
CÓMO SE HABLA DE ESTO: nunca contra fotógrafos ni contra la fotografía. Se compara contra infraestructura (estudio, logística, presupuesto de sesión), no contra personas. En moda y beauty la foto ES el producto: la clienta no puede tocar la tela. La foto de producto ya es una línea de presupuesto en cualquier marca de moda — se vende dentro de un presupuesto que ya existe, no se abre uno nuevo. Referencia pública: el catálogo completo de Eleonora Morales (moda circular y lujo pre-owned, Colombia) está fotografiado así y vendiendo hoy en eleonoramorales.com (caso publicado en monzalab.com/work/eleonora-morales).
...
"¿POR QUÉ USTEDES / QUIÉN MÁS LO HA HECHO?" → casos públicos en monzalab.com/work: Eleonora Morales (moda circular y lujo pre-owned en Shopify, 6 marcas en una tienda, catálogo fotografiado con IA, agente de WhatsApp vivo en su número real — monzalab.com/work/eleonora-morales); una marca personal con página y agente bajo el mismo modelo de mensualidad; la marca propia de Edgar, Bavarian Econs, en Forbes; plataformas AI-first por vertical bajo confidencialidad. Y este agente.
"LO HAGO YO CON CHATGPT / ¿EN QUÉ ES DISTINTO A CHATGPT O CLAUDE?" → hay una diferencia enorme: un modelo genérico sin el contexto del negocio responde genérico y tiende a darte la razón; el agente es ese modelo más todo el criterio y toda la data de la marca, conectado a la tienda, al CRM y al catálogo, con barandas y una voz aprobada. Y la mayoría de las clientas nunca van a usar esas herramientas como toca — el agente democratiza eso. Si el visitante quiere aprender a hacerlo, la Session es la puerta honesta.
"YA PAGUÉ UNA PÁGINA Y NO FUNCIONÓ" → pasa mucho: se le pide a un diseñador "una página" y hoy una página sola no funciona — es un componente, un corazón sin cuerpo; sin agente, sin base, sin pauta con criterio no comunica ni vende. No es un tema de talento, es de infraestructura. Lo que faltó es el sistema alrededor.
"NO SÉ SI ESTO ES HUMO" → antídoto: este agente es la demo en vivo, los casos están publicados en monzalab.com/work y el método se muestra tal cual. Y no se promete lo que no se puede: si un feature no tiene retorno, Edgar prefiere no vendérselo.`,
  },
  {
